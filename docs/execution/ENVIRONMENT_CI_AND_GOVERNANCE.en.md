...
GitHub API: public repository, default master; Actions enabled; self-hosted runner count0 (this does not mean hosted runners unavailable). Existing G2 workflow active; latest queried develop run35654063756 succeeded at fb63a96ef06cc0375fca9dbceb4f59e89e7157a7. Current branch has no new remote readiness run because this preparation has not pushed. master/develop protected=false; both protection APIs return404 'Branch not protected'; parent/repository rulesets=[] and matching branch rules=[]. This is verified absence, not a permission inference. The repository API reports the current identity permissions admin=true and push=true; it is not evidence of an Agent identity restricted from merge/release. Actions permits all actions and does not require SHA pinning, so action supply-chain hardening remains an Owner choice.
...
Those existing contexts currently cover only PRs targeting develop. Requiring them unchanged on master would leave master PRs waiting indefinitely for jobs that never trigger. Enable `verify`/`assets` for develop after actual execution. For master, first establish an Owner-reviewed RC workflow covering `pull_request: master`, read-only acceptance and Owner-managed candidate branches, then require its observed exact contexts. The current harness work-branch restrictions also cannot be applied unchanged to read-only develop-to-master RC validation; explicitly review the verification scope at that point. Check trigger/branch applicability, observe real contexts, then enable required checks. Never fill the gap with bypass or always-successful jobs. This preparation does not modify the frozen G2 workflow or create a fake RC job.
