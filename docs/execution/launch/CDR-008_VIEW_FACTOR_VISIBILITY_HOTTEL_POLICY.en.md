...
Status: **PROPOSED — AWAITING OWNER REVIEW**. Date: 2026-09-22. Related: DEV-028, LCB-01; existing CDR-003/006 Geometry contracts are preserved. This proposal does not rewrite M06/M09 or approved P3; approval would authorize a separately traceable specification supplement before production implementation.

## Decision requested

Adopt actual-normal, visible-ray geometric F on the unchanged approved finite Geometry for the complete tilt [0°,180°] domain. Use a unified orientation-independent exchange definition, not an angle-specific fix. Require corrected Fast geometric groups to aggregate the same finite exchange. Preserve raw Reference observations, approve corrected expectations only by exact manifest, and retain independent checks in addition to Reference differential.

Default recommendation: **adopt this policy**, including the finite Fast aggregation. Alternatives are: (a) retain raw Reference only as an explicitly named diagnostic compatibility result outside physical F; or (b) defer approval and keep launch blocked while refining the same mathematical policy. Reject shrinking the approved domain, swapping front/back, replacing negative values by zero, global abs, and widening tolerance to accept nonphysical values. The small empirical abs-helper patch is not a general proof and leaves Fast's positive failures unresolved.
...
- Physical rows retain the existing `F_i,sky=1−sum(physical F_i,*)`; sky row stays zero and has no length reciprocity. With finite ground this is an open-boundary residual and includes escape outside the ground extent, even for downward receivers. This proposal does not silently reinterpret it as only the astronomical upper hemisphere. Check nonnegativity and closure; do not clamp an invalid residual.

## Full/Fast compatibility and API

Full F uses the preceding definition. Proposed Fast ground/PV/shaded/illuminated geometric groups use sums of those same finite exchanges; for a union of receiver segments, use length-weighted aggregation. Direct group computation is allowed if proved equivalent. This replaces the finite-inconsistent low-endpoint proxy and out-of-domain edge segments; it does not alter Fast's one-bounce approximation into a Full solve or close CDR-007 by implication.

Public Geometry APIs and identities do not change. F dimensions, receiver/source axes, inactive slots and residual boundary remain stable. Physical numerical results intentionally change in DEV-028 cases and finite Fast groups; preserve per-field deviations. AOI G is a separate operator and is not constrained to F's [0,1] rule by this CDR. No claim is made that all P4/P5/P6/P7 fixtures or public result-availability decisions are now approved.

## Expected values, tolerance and acceptance

Raw/normalized Reference remain immutable evidence. Corrected values may enter an approved oracle only after Owner approval of this decision, DEV-028, the exact manifest and comparator policy. A Rust output can never generate its own expected value. No candidate auto-promotion occurs.

The LCB-01 candidate has 66 cases/198 artifacts. Bounds and closure/length reciprocity allowances are proposed at1e−10; expected absolute comparison is2e−9. These are research numerical allowances derived from requested quadrature accuracy, not algorithm thresholds or changes to `geometry-tolerance-v0.1`. The observed independent errors are much smaller, but do not constitute a universal platform proof. Replacing the research budget with production acceptance requires evidence and explicit approval, not silent editing during a failing implementation.

Required verification: all active physical and residual entries within bounds; valid exact support rules; closure only on active receivers; physical length reciprocity; independent Hottel and line/tensor cases; occlusion, full-domain mirror and endpoint-reversal metamorphics; unchanged Geometry; exact case inventory and generator/manifest integrity. Reciprocity/closure alone are insufficient, particularly when generated by construction. Mutation tests must reject negative, >1, reciprocal-but-negative and closed-but-invalid matrices. Missing artifacts, missing cases or absent production tests cannot pass a milestone.

## Owner closure record

Review [investigation report](LCB-01_VF_INVESTIGATION_REPORT.en.md), [DEV-028](DEV-028_VIEW_FACTOR_ORIENTATION_CANDIDATE.en.md), `reference/candidate/vf-lcb01-v0.1/manifest.json`, its proposed profile and `evidence/execution/launch-closure/lcb-01/candidate-index.json` together. Required response: approve/revise/reject this unified physical policy; explicitly accept/reject finite Fast aggregation; identify approved manifest/profile hashes or request replacement evidence. Approval is recorded by the Owner, never inferred from this recommendation or a test PASS.

Until then: **PROPOSED — AWAITING OWNER REVIEW**, LCB-01 ready for decision but open for launch; whole-project closure remains BLOCKED. After Owner closure, resume the remaining P4–P7 research/oracle preparation under the applicable authorization. Production launch remains a separate decision. This investigation ends without executing the future implementation task.
